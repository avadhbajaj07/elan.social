...
export default function ClientPortalPage() {
  ...
  useEffect(() => {
    // Search localStorage clients first, fallback to INITIAL_CLIENTS
    const saved = loadClientsFromStorage();
    const all = saved.length > 0 ? saved : INITIAL_CLIENTS;
    const found = all.find(
      (c) => c.slug === slug || c.id === slug || c.name.toLowerCase().replace(/[^a-z0-9]/g, "-") === slug
    );
  ...
  if (!client) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center font-sans p-4">
        <div className="max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 text-center space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-red-500/10 text-red-400 flex items-center justify-center mx-auto">
            <Lock className="w-6 h-6" />
          </div>
          <h1 className="text-xl font-black">Portal Not Found</h1>
          <p className="text-xs text-slate-400 font-bold leading-relaxed">
            The workspace portal link you requested does not exist or has expired. Please check with your agency.
          </p>
        </div>
      </div>
    );
        ...
        <div className="max-w-6xl mx-auto px-4 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-emerald-400 font-black bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded-full flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5" /> Client Access Live
            </span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 pt-8 space-y-8">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-purple-950/40 to-slate-900 border border-purple-500/20 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-2xl">
          ...
          <div className="flex items-center gap-2 bg-slate-900/80 border border-slate-800 rounded-2xl p-3 text-xs">
            ...
            <span className="text-slate-300 font-bold">Auto-Syncing with Instagram & Social API</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Followers", value: (client.stats?.totalFollowers || 24500).toLocaleString(), change: "+12.4%", color: "text-emerald-400" },
            { label: "Monthly Impressions", value: (client.stats?.monthlyImpressions || 184000).toLocaleString(), change: "+28.1%", color: "text-purple-400" },
            { label: "Posts Published", value: (client.stats?.postsThisMonth || 18).toString(), change: "This month", color: "text-blue-400" },
            { label: "Growth Rate", value: `${client.stats?.followerGrowth || 8.4}%`, change: "MoM", color: "text-pink-400" },
          ].map((stat, idx) => (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            <div className="bg-slate-950 border border-pink-500/30 rounded-2xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-sm text-white">Instagram Business</h3>
                  <p className="text-[11px] text-slate-400 font-mono">@{client.slug || client.name.toLowerCase().replace(/[^a-z0-9]/g, "")}</p>
                </div>
              </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4">
          ...
          <div className="space-y-3">
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-purple-400 font-black shrink-0">
                  📷
                </div>
                <div>
                  <span className="text-[10px] bg-emerald-500/20 text-emerald-300 font-black px-2 py-0.5 rounded-full uppercase tracking-wider">
